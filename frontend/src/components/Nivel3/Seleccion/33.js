...
//Permite avanzar entre ejercicios
  ...
  const checkAnswer = () => {
    if (selectedAnswer === "while") {
      setOutput("¡Correcto! La palabra clave 'while' es la correcta.");
      setScore(score + 10);
      setShowNext(true);
    } else {
...
print(f"La suma de los números ingresados es: {suma}")`}
                ...
                <div className="options">
                  {["for", "while", "if", "switch"].map((option) => (
                    <div
                      key={option}
...
export default Nivel3Treintatres;
