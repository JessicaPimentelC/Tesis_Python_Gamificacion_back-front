...
const handleNext = async () => {
  ...
  const checkAnswer = () => {
    if (printFunction.toLowerCase() === 'print') {
      setOutput('¡Correcto! La función usada es print.');
      setScore(score + 10);
    } else {
...
const handleVerify = async () => {
  ...
  const userInput = printFunction.trim().toLowerCase();
...
export default Nivel2Diecisiete;
